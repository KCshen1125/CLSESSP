...
class ModelArguments:
    ...
    prompt_len: int = field(
        default=0,
        metadata={
            "help": "Length of Continuous Prompt."
        }
    )
...
def main():
    ...
    # Unsupervised datasets with W-KCSE
    #sentence,sentence for weak_pos,sentence for strong_pos
    ...
    def prepare_features(examples):
        ...
        examples['prompts'] = []
        examples['weak_pos_prompts'] = []
        examples['strong_pos_prompts'] = []
        examples['neg_examples'] = []
        examples['neg_prompts'] = []
        ...
        num = model_args.prompt_len
        _prompt = ["", " ".join([" "] + ["[PAD]" for _ in range(num)] +  ["[MASK] ."]).replace('[PAD]', pad_token).replace('[MASK]', mask_token)]
        ...
        for idx in range(total):
            ...
            sent0 = examples[sent0_cname][idx]
            sent1 = examples[sent1_cname][idx]
            sent2 = examples[sent2_cname][idx]
            ...
            examples[sent0_cname][idx] = sent0
            ...
            examples[sent1_cname][idx] = sent1

            nid = random.randint(0, len(strong_pos_prompts)-1)
            examples['strong_pos_prompts'].append(strong_pos_prompts[nid])
            examples[sent2_cname][idx] = sent2

            nid = random.randint(0, len(neg_prompts)-1)
            examples['neg_examples'].append(sent0)
            examples['neg_prompts'].append(neg_prompts[nid])



        sentences = examples[sent0_cname] + examples[sent1_cname] + examples[sent2_cname] + examples['neg_examples']

        prompts = examples['prompts'] + examples['weak_pos_prompts'] + examples['strong_pos_prompts'] + examples['neg_prompts']
    ...
    class OurDataCollatorWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            special_keys = ['input_ids', 'attention_mask', 'token_type_ids', 'mlm_input_ids', 'mlm_labels', 'prompt_mask']
            bs = len(features)
            if bs > 0:
            ...
            if model_args.do_mlm:
                batch["mlm_input_ids"], batch["mlm_labels"] = self.mask_tokens(batch["input_ids"], batch["prompt_mask"])
        ...
        def mask_tokens(
            self, inputs: torch.Tensor, prompt_mask: torch.Tensor, special_tokens_mask: Optional[torch.Tensor] = None
            ...
            masked_indices = torch.bernoulli(probability_matrix).bool() & ~prompt_mask.bool()
