...
if __name__=="__main__":
    # 加载CSV文件
    file_path = "/hy-tmp/promptCSE-main/promptCSE-main/ConPVP/SentEval/data/downstream/STS/STSBenchmark/sts-train.csv"  # 请替换为您的文件路径
    data = pd.read_csv(file_path, delimiter='\t', header=None, nrows=2000,
                       names=["source", "type", "year", "id", "score", "sentence1", "sentence2"])

    # 使用SimCSE编码句子对
    model_name = "/hy-tmp/promptCSE-main/promptCSE-main/ConPVP/model_path/roberta-large"
    pooler = "cls"
    simcse = SimCSE(model_name, pooler=pooler)

    # 计算句子对的余弦相似度
    predicted_scores = []
    for _, row in data.iterrows():
        similarity = simcse.similarity(row["sentence1"], row["sentence2"])
        predicted_scores.append(similarity)

    # 将预测的相似度与真实的STS得分进行比较
    real_scores = data["score"].tolist()

    min_val = np.min(predicted_scores)
    max_val = np.max(predicted_scores)

    normalized_similarities = (predicted_scores - min_val) / (max_val - min_val)

    # 绘制散点图
    plt.scatter(real_scores, normalized_similarities, alpha=0.6, color='brown', marker='o', s=10)
    #brown
    #blue
    #orange
    #green
    plt.xlabel('the gold similarity score', fontsize=15)
    plt.ylabel('the predicted cosine similarity', fontsize=15)
    plt.title('RoBERTa-large ([CLS])', fontsize=15)
    #RoBERTa-large ([CLS])
    #RoBERTa-large +SimCSE([CLS])
    plt.grid(True)
    # 保存图形为 .png 文件
    plt.savefig('RoBERTa.png', dpi=300)  # dpi 参数设置图像的分辨率
